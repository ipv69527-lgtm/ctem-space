...
export default function AssetDetail() {
  ...
  const longitude = rawValue(asset, ['longitude', 'lng']);
  const latitude = rawValue(asset, ['latitude', 'lat']);
  const manufacturer = appInfoValue(asset, ['manufacturer', 'manufacturer_short']) || rawValue(asset, ['manufacturer']);
  const brand = appInfoValue(asset, ['brand']) || rawValue(asset, ['brand']);
  const model = appInfoValue(asset, ['model']) || rawValue(asset, ['model']);
  const product = appInfoValue(asset, ['name']) || rawValue(asset, ['product', 'app', 'title']);
  const country = rawValue(asset, ['country']);
  const province = rawValue(asset, ['province']);
  const city = rawValue(asset, ['city']);
  const device = rawValue(asset, ['device']);
  const deviceType = rawValue(asset, ['device_type', 'category_sub']);
